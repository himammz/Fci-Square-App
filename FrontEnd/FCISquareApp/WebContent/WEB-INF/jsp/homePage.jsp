...
<link rel="shortcut icon" type="image/x-icon" href="/FCISquareApp/images/mobile-icon-md.ico" />
 <link rel="stylesheet" href="/FCISquareApp/stylesheets/showHome.css">
	...
	<div id="cssbar">
 	...
 	<div id="allcheckins">
		<c:forEach var="i" begin="0" end = "${it.size()}">
			<c:if test="${it[i].Time ne null}" >
				
				<div class="checkin">
				
					<h5 id="name">${it[i].UserName} <p><span style = "color :#fd8a17">checked-in</span> ${it[i].PlaceName}</p> </h5>
					<p id ="desc">${it[i].Description}</p>
					<p id="time">${it[i].Time}</p>
				</div>
			</c:if>
		</c:forEach>
	 ...
	 <div class="savedList">
	 	<h1>Friend's Saved List</h1>
	 	<hr>
	 	<hr>
		<c:forEach var="i" begin="1" end = "${it.size()}">
			<c:if test="${it[i-1].Time eq null}" >
					<h5 id="name">${it[i-1].UserName} <p> <span style = "color :#fd8a17">saved</span>  ${it[i-1].PlaceName} </p> </h5>
					<p id ="desc"> <span style = "font-weight: bold">description:</span> ${it[i].Description}</p>
	 				<hr>
